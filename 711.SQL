...
left outer join dim_product on stage_target_data_product.product_id = dim_product.productid
...
left outer join dim_channel on lower(replace(dim_channel.ChannelName, '-', '')) = lower(replace(stage_target_data_channel.Channel_Name, '-', '')) -- cleans up naming inconsistencies
left outer join dim_date  on dim_date.year = stage_target_data_channel.year
   and dim_date.month_num_in_year = 1
